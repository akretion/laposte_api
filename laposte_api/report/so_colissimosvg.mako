...
^FO40,345^PR2,2^BCN,230,N,N,N^FD${d['suivi_barcode'][:4].replace(' ','') + '>5'}${d['suivi_barcode'][4:].replace(' ','')}^FS
^FO40,575^GB402,3,4^FS
^FO90,585^FDN° de colis : ${d['suivi_barcode']}^FS

% if _product_code == '6J':
^FO670,370^BY3,,80^BCR,100,N,N,N^FD>:${s['account_chargeur'][0:1] + '>5'}${s['account_chargeur'][1:]}^FS
^FO650,460^A0B,18^FD${s['account_chargeur']}^FS
% endif
% endif
...
^FO70,880^BCN,230,N,N,N^FD${d['prise_en_charge_barcode'] and d['prise_en_charge_barcode'][:9].replace(' ','') + '>5' + d['prise_en_charge_barcode'][9:].replace(' ','') or ''}^FS
^FO230,1120^FDN° PCH:  ${d['prise_en_charge_barcode']}^FS
